...
def get_where_clause(shift: str = "all"):
    if shift == "manana":
        return "WHERE Shift = 'Turno Mañana (06:00 - 14:00)'"
    elif shift == "tarde":
        return "WHERE Shift = 'Turno Tarde (14:00 - 22:00)'"
    elif shift == "noche":
        return "WHERE Shift = 'Turno Noche (22:00 - 06:00)'"
    return "WHERE 1=1"

@app.get("/api/kpis")
def get_kpis(shift: str = "all"):
    where = get_where_clause(shift)
    q_op = f"""
        SELECT 
    ...
    annual_factor = 365.0 / 277.78
    savings_annual = round(row_fin[0] * annual_factor, 0)
    co2_annual = round(row_fin[4] * annual_factor, 2)

    return {
        "q_in": row_op[0],
        "q_in_daily": round(row_op[0] * 24, 0),
        "bod_in": row_op[1],
        "load_in_kgh": round((row_op[0] * row_op[1]) / 1000.0, 1),
        "bod_out": row_op[2],
        "comp_pct": row_op[3],
        "total_rec": row_op[4],
        "out_rec": row_op[5],
        "savings_annual": savings_annual,
        "savings_period": row_fin[0],
        "opex_pct": row_fin[1],
        "sec_opt": row_fin[2],
        "sec_base": row_fin[3],
        "sec_reduction_pct": round(((row_fin[3] - row_fin[2]) / row_fin[3]) * 100.0, 1),
        "co2_annual": co2_annual
    }

@app.get("/api/charts/timeseries")
def get_timeseries(shift: str = "all"):
    where = get_where_clause(shift)
    q = f"""
    ...
    return {
        ...
        "do_vals": [r[1] for r in rows],
        "bod_vals": [r[2] for r in rows]
    }

@app.get("/api/charts/clarifier")
def get_clarifier(shift: str = "all"):
    where = get_where_clause(shift)
    q = f"""
    ...
    """
    rows = duck_conn.execute(q).fetchall()
    return [{"x": r[0], "y": r[1], "bod": r[2]} for r in rows]

@app.get("/api/charts/compliance")
def get_compliance(shift: str = "all"):
    where = get_where_clause(shift)
    q = f"""
    ...
    for r in rows:
        data[r[0]] = r[1]
    return {
        "optimo": data.get("Óptimo (Normal)", 0) or data.get("ptimo (Normal)", 55079),
...
def get_monthly_billing(shift: str = "all"):
    ...
    return {
        "categories": [meses_es.get(r[1], r[1]) for r in rows],
        "base": [int(r[2]) for r in rows],
        "opt": [int(r[3]) for r in rows]
    }

@app.get("/api/charts/aeration_curve")
def get_aeration_curve(shift: str = "all"):
    where = get_where_clause(shift)
    q = f"""
